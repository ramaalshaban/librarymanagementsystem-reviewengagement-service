...
class GetReviewMcpController extends ReviewEngagementMcpController {
  constructor(params) {
    super("getReview", "getreview", params);
    this.dataName = "review";
    this.crudType = "get";
  }

  createApiManager() {
    return new GetReviewManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    parameters: GetReviewMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const getReviewMcpController = new GetReviewMcpController(mcpParams);
      try {
        const result = await getReviewMcpController.processRequest();
        //return GetReviewMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",
