...
class UpdateReviewMcpController extends ReviewEngagementMcpController {
  constructor(params) {
    super("updateReview", "updatereview", params);
    this.dataName = "review";
    this.crudType = "update";
  }

  createApiManager() {
    return new UpdateReviewManager(this.request, "mcp");
  }
  ...
  static getInputScheme() {
    return {
      ...
      reviewText: z.string().optional().describe("Full review content."),
...
module.exports = (headers) => {
  return {
    ...
    parameters: UpdateReviewMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const updateReviewMcpController = new UpdateReviewMcpController(
        mcpParams,
      );
      try {
        const result = await updateReviewMcpController.processRequest();
        //return UpdateReviewMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",
