...
class DbListReviewsCommand extends DBGetListMongooseCommand {
  constructor(input) {
    super(input);
    this.commandName = "dbListReviews";
    this.emptyResult = true;
    this.objectName = "reviews";
  ...
  async transposeResult() {
    for (const review of this.dbData.items) {
  ...
  createQuery() {
    ...
    return Review.find(this.whereClause);
  }
  ...
  async getCqrsJoins(item) {
    if (Review.getCqrsJoins) {
      await Review.getCqrsJoins(item);
    }
  }

  async setPaginationTotalRowCount() {
    this.paginationTotalRowCount = await Review.countDocuments(
      this.whereClause,
    );
  }
...
const dbListReviews = (input) => {
  const dbGetListCommand = new DbListReviewsCommand(input);
  return dbGetListCommand.execute();
};

module.exports = dbListReviews;
