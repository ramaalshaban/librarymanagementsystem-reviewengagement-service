...
const getReviewStatsByQuery = async (query, stats) => {
  const promises = [];
  const statLabels = [];
  try {
    ...
    for (const stat of stats) {
      ...
      if (stat === "count") {
        promises.push(Review.countDocuments(queryWithSoftDelete));
        ...
        if (statParts[0] === "sum") {
          ...
          promises.push(
            Review.aggregate(pipeline).then((result) =>
    ...
    if (promises.length == 0) {
      return await Review.countDocuments(queryWithSoftDelete);
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenRequestingReviewStatsByQuery",
      err,
    );
  }
};

module.exports = getReviewStatsByQuery;
