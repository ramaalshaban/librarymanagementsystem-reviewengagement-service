...
const getRecommendationStatsByQuery = async (query, stats) => {
  const promises = [];
  const statLabels = [];
  try {
    ...
    for (const stat of stats) {
      ...
      if (stat === "count") {
        promises.push(Recommendation.countDocuments(queryWithSoftDelete));
        ...
        if (statParts[0] === "sum") {
          ...
          promises.push(
            Recommendation.aggregate(pipeline).then((result) =>
    ...
    if (promises.length == 0) {
      return await Recommendation.countDocuments(queryWithSoftDelete);
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenRequestingRecommendationStatsByQuery",
      err,
    );
  }
};

module.exports = getRecommendationStatsByQuery;
